              ...
              <div className="space-y-4">
                {opportunities?.slice(0, 5).map(opp => (
                  <div key={opp.id} className="flex justify-between items-center text-sm border-b border-border pb-2 last:border-0 last:pb-0">
                    <div>
                      <span className="font-bold">{opp.asset}</span>
                      <div className="text-xs text-muted-foreground truncate max-w-[200px]">{opp.summary}</div>
                    </div>
                    <div className={`px-2 py-1 rounded text-xs font-bold uppercase ${opp.recommendation === "buy" ? "bg-green-500/20 text-green-500" : opp.recommendation === "sell" ? "bg-red-500/20 text-red-500" : "bg-yellow-500/20 text-yellow-500"}`}>
                      {opp.recommendation}
                    </div>
                  </div>
                ))}
