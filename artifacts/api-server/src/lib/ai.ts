...
export async function generateMarketAnalysis(asset: string): Promise<{
  ...
  const systemPrompt = `You are TradeMind AI, an expert crypto market analyst. Analyze the given asset and return a JSON object with these exact fields:
{
...
Return ONLY valid JSON, no markdown, no explanation.`;
  ...
  const content = await callQwen(
    [{ role: "user", content: `Analyze ${asset} crypto asset for trading opportunity. Use realistic but simulated data.` }],
    systemPrompt
  );
  ...
  try {
    ...
    logger.warn({ asset, content }, "Failed to parse Qwen analysis JSON, using mock");
    return getMockAnalysis(asset);
  }
}
...
function getMockAnalysis(asset: string): ReturnType<typeof generateMarketAnalysis> extends Promise<infer T> ? T : never {
  ...
  return {
    bullishScore: isBullish ? 72 + Math.floor(Math.random() * 20) : 35 + Math.floor(Math.random() * 25),
    bearishScore: isBullish ? 25 + Math.floor(Math.random() * 15) : 60 + Math.floor(Math.random() * 20),
    ...
    onchainSignals: {
      whaleActivity: isBullish ? "Net accumulation — 12,400 units moved to cold storage" : "Exchange deposits rising — potential sell pressure",
...
export async function generateTradeReplay(params: {
  ...
  return {
    ...
    actualOutcome: isLoss
      ? `Trade closed at a loss of ${Math.abs(params.pnl).toFixed(2)} (${((params.exitPrice - params.entryPrice) / params.entryPrice * 100).toFixed(2)}%). Price moved against position after entry.`
...
export async function generateChatResponse(
  ...
  const systemPrompt = `You are TradeMind AI, an expert AI trading mentor and analyst. You have deep knowledge of crypto markets, technical analysis, behavioral finance, and trading psychology.
